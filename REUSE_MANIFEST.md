...
|---|---|---|---|---|---|
| 1 | `crates/russell-meta/src/help.rs` + `crates/russell-meta/src/hkask_adapter.rs` | `slate/stack/crates/stack-llm/src/openai.rs` + `src/wire.rs` | `67a13834d8af4efa8c330ce10ef1031bf2cdeee2` | Uses Russell's `DoctorError` + SOAP bundle composition. Drops streaming, tool-calling, structured-output, retry. Adds per-request ZDR enforcement. Refactored from original `openrouter.rs` into `help.rs` (Nurse pipeline) + `hkask_adapter.rs` (InferencePort adapter). | Review on upstream bug fix; pull fixes, not features. Log changes in §6. |
