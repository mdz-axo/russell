...
pub fn resolve(response: &str, skills: &[Skill]) -> Option<std::result::Result<ResolvedAction, ActionError>> {
    let action_line = response.lines().rev().find(|line| line.trim().starts_with("ACTION:"))?;

    let raw = action_line.trim();
    let spec = match raw.strip_prefix("ACTION:") {
        Some(s) if !s.trim().is_empty() => s.trim(),
        _ => return Some(Err(ActionError::MalformedPrefix { raw: raw.to_string() })),
    };

    let (skill_id, action_id) = match spec.split_once('/') {
        Some((a, b)) if !a.trim().is_empty() && !b.trim().is_empty() => (a.trim(), b.trim()),
        Some((_, _)) => return Some(Err(ActionError::EmptyId { spec: spec.to_string() })),
        None => return Some(Err(ActionError::MissingSeparator { spec: spec.to_string() })),
    };
...
mod tests {
    use super::*;
    use russell_skills::{Probe, Intervention, Safety};
    ...
    fn resolves_probe() {
        let skills = [make_skill()];
        let result = resolve("ACTION: test-skill/probe-1", &skills).unwrap().unwrap();
    ...
    fn resolves_intervention() {
        let skills = [make_skill()];
        let result = resolve("ACTION: test-skill/iv-1", &skills).unwrap().unwrap();
        assert!(!result.is_probe());
    ...
    fn unknown_skill_is_error() {
        let skills = [make_skill()];
        let err = resolve("ACTION: bad-skill/probe-1", &skills).unwrap().unwrap_err();
