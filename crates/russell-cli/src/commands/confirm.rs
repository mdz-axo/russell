...
use russell_core::{paths::Paths, Event, Severity};
...
pub fn list_pending(paths: &Paths) -> Result<()> {
    ...
    for row_result in rows {
        if !has_events {
            ...
            println!();
            println!("{:<8} {:<20} {:<10} {:<30}", "ID", "Probe", "Severity", "Intervention");
            println!("{}", "-".repeat(78));
            has_events = true;
        }
        
        let (event_id, probe, severity, intervention, _summary) = row_result?;
        println!("{:<8} {:<20} {:<10} {:<30}", event_id, probe, severity, intervention);
    }
...
pub fn confirm_event(paths: &Paths, event_id: i64, deny: bool) -> Result<()> {
    ...
    let event = journal.get_event(event_id)
        .context("failed to fetch event")?;
    
    if event.action != "reflex_proposed" {
        anyhow::bail!("Event {} is not a reflex_proposed event (action: {})", event_id, event.action);
    }
    
    let probe = event.outputs.get("probe").and_then(|v| v.as_str()).unwrap_or("unknown");
    let intervention = event.outputs.get("intervention").and_then(|v| v.as_str()).unwrap_or("unknown");
    ...
    if deny {
        ...
        denial_event.summary = Some(format!("Operator denied reflex arc for probe '{}' (intervention: {})", probe, intervention));
        denial_event.outputs = event.outputs.clone();
        denial_event.outputs.insert("original_event_id".into(), serde_json::json!(event_id));
        ...
        approval_event.summary = Some(format!("Operator approved reflex arc for probe '{}' (intervention: {})", probe, intervention));
        approval_event.outputs = event.outputs.clone();
        approval_event.outputs.insert("original_event_id".into(), serde_json::json!(event_id));
...
pub fn run(paths: &Paths, args: &[String]) -> Result<()> {
    ...
    let event_id: i64 = first_arg.parse()
        .context("event ID must be a number")?;
